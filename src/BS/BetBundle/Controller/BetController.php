...
class BetController extends Controller
{
    ...
    public function difference5SecondRankAction()
    {
        ...
        $strategy = $strategyRepository->getByLabel("Difference5SecondRank")[0];
            ...
            foreach ($offerList as $offer) {
                if($offer->getEnd() > '2016-02-26') {
                    if (($offer->getHomeTeamId() != null) && ($offer->getOutsideTeamId() != null)) {
                        ...
                        if(abs(($homeTeam->getPoints() - $outsideTeam->getPoints())) < 6) {
                            if ($homeTeam->getRank() < $outsideTeam->getRank()) {
    ...
    public function perCentVictory50For5Action()
    {
        ini_set('max_execution_time', 18000);
        ini_set('memory_limit', '-1');
        $betRepository = $this->getDoctrine()->getManager()->getRepository('BSBetBundle:Bet');
        ...
        $strategyRepository = $this->getDoctrine()->getManager()->getRepository('BSResultBundle:Strategy');
        $competitionList = $teamRepository->getTeamGroupByCompetition();
        $strategy = $strategyRepository->getByLabel("PerCentVictory50For5")[0];
        foreach($competitionList as $competition) {
            $offerList = $offerRepository->getOfferByCompetitionId($competition['competitionId']);
            $ranking = $teamRepository->getRanking($competition['competitionId']);
            $split = floatval(sizeof($ranking) / 2.00);
            foreach ($offerList as $offer) {
                if($offer->getEnd() > '2016-02-26') {
                    if (($offer->getHomeTeamId() != null) && ($offer->getOutsideTeamId() != null)) {
                        $homeTeam = $teamRepository->find($offer->getHomeTeamId());
                        $outsideTeam = $teamRepository->find($offer->getOutsideTeamId());
                        if(strlen($homeTeam->getSeries()) > 5)
                        {
                            $homeSeriesRatio = floatval(substr_count(substr($homeTeam->getSeries(), 0, 5), 'V')/5.00);
                        }
                        else
                        {
                            $homeSeriesRatio = floatval(substr_count($homeTeam->getSeries(), 'V')/(strlen(substr($homeTeam->getSeries(), 0, 5))*1.00));
                        }

                        if(sizeof($outsideTeam->getSeries()) > 5)
                        {
                            $outsideSeriesRatio = floatval(substr_count(substr($outsideTeam->getSeries(), 0, 5), 'V')/5.00);
                        }
                        else
                        {
                            $outsideSeriesRatio = floatval(substr_count($outsideTeam->getSeries(), 'V')/(strlen(substr($outsideTeam->getSeries(), 0, 5))*1.00));
                        }
                        var_dump($homeTeam->getSeries());
                        var_dump($homeSeriesRatio);
                        var_dump($outsideTeam->getSeries());
                        var_dump($outsideSeriesRatio);

                        if(($outsideSeriesRatio >= 0.5) || ($homeSeriesRatio >= 0.5))
                        {
                            var_dump($offer);
                            if($homeSeriesRatio >= $outsideSeriesRatio)
                            {
                                $outcome = $outcomeRepository->getOutcomeByLabelAndOffer('Domicile', $offer);
                                if (!empty($outcome)) {
                                    $outcome = $outcome[0];
    ...
    public function sameHalfSecondRankAction()
    {
        ini_set('max_execution_time', 18000);
        ini_set('memory_limit', '-1');
        $betRepository = $this->getDoctrine()->getManager()->getRepository('BSBetBundle:Bet');
        ...
        $competitionList = $teamRepository->getTeamGroupByCompetition();
        $strategy = $strategyRepository->getByLabel("SameHalfSecondRank")[0];
        foreach($competitionList as $competition) {
            $offerList = $offerRepository->getOfferByCompetitionId($competition['competitionId']);
            $ranking = $teamRepository->getRanking($competition['competitionId']);
            $split = floatval(sizeof($ranking) / 2.00);
            foreach ($offerList as $offer) {
                if($offer->getEnd() > '2016-02-26') {
                    if (($offer->getHomeTeamId() != null) && ($offer->getOutsideTeamId() != null)) {
                        $homeTeam = $teamRepository->find($offer->getHomeTeamId());
                        $outsideTeam = $teamRepository->find($offer->getOutsideTeamId());
                        if(($homeTeam->getRank()-$split)*($outsideTeam->getRank()-$split) > 0) {
                            if ($homeTeam->getRank() < $outsideTeam->getRank()) {
                                    $outcome = $outcomeRepository->getOutcomeByLabelAndOffer('Exterieur', $offer);
                                    if (!empty($outcome)) {
                                        $outcome = $outcome[0];
    ...
    public function secondAtTheRankAction()
    {
        ini_set('max_execution_time', 18000);
        ini_set('memory_limit', '-1');
        $betRepository = $this->getDoctrine()->getManager()->getRepository('BSBetBundle:Bet');
        ...
        $competitionList = $teamRepository->getTeamGroupByCompetition();
        $strategy = $strategyRepository->getByLabel("SecondAtTheRank")[0];
        foreach($competitionList as $competition) {
            $offerList = $offerRepository->getOfferByCompetitionId($competition['competitionId']);
            $ranking = $teamRepository->getRanking($competition['competitionId']);
            foreach ($offerList as $offer) {
